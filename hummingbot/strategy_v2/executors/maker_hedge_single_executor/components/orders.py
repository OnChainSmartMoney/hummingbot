...
class OrdersHelper:
    ...
    def compute_limit_price(self, side: TradeType) -> Optional[Decimal]:
        ...
        base_price = self.exe.get_price(
            self.exe.maker_connector, self.exe.maker_pair, price_type
        )
        ...
        if side == TradeType.BUY:
            self.exe.logger().info(
                f"[Order] Limit price: {base_price * (Decimal('1') - off)}"
            )
    ...
    async def validate_sufficient_balance(self):
        mid = self.exe.get_price(
            self.exe.maker_connector, self.exe.maker_pair, PriceType.MidPrice
        )
        ...
        self.exe.logger().info(
            f"[Config] pair_notional_usd_cap: {self.exe.pair_notional_usd_cap}, per_order_max_notional_usd: {self.exe.per_order_max_notional_usd}, leverage: {self.exe.leverage}"
        )
        ...
        if not self.exe.is_enough_maker_cap(remaining_cap):
            self.exe.logger().warning(
                f"[Validation] Not enough remaining maker cap to place new order, remaining cap: ${remaining_cap:.2f}, need at least: ${self.exe.per_order_min_notional_usd:.2f}. Completing executor."
            )
            return

        next_notional_usd = min(
            remaining_cap, Decimal(str(self.exe.per_order_max_notional_usd))
        )
        leverage = (
            self.exe.leverage
            if getattr(self.exe, "leverage", None) is not None
            else Decimal("1")
        )
        if leverage <= 0:
            leverage = Decimal("1")
        ...
        self.exe.logger().info(
            f"[Validation] connector {self.exe.maker_connector} requires ~{required_margin:.6f} {quote}"
        )
    ...
    async def place_next_part(self):
        ...
        if exe._closing:
            ...
            if remaining_cap <= 0:
                exe._finalize_hedge_tail()
                exe.logger().info(
                    "[Order] Pair notional USD cap fully consumed; completing executor."
                )
                exe.close_type = CloseType.COMPLETED
                exe.stop()
                return
            ...
            if px_preview is None or px_preview <= 0:
                return
            next_notional_usd = min(
                remaining_cap, Decimal(str(exe.per_order_max_notional_usd))
            )
            exe.logger().info(
                f"[Order] Next notional USD: ${next_notional_usd:.2f} at limit px {px_preview:.8f}"
            )
            ...
            if maker_min_notional and next_notional_usd < maker_min_notional:
                exe.logger().info(
                    f"[Order] Skip maker OPEN: below min notional ${maker_min_notional}"
                )
                return
            mode_desc = "OPEN"
        ...
        if not exe._closing:
            ...
            if planned_notional > rem_cap_now:
                try:
                    entry_conn = exe._strategy.connectors[exe.maker_connector]
                    adjusted = entry_conn.quantize_order_amount(
                        exe.maker_pair, amount * Decimal("0.999")
                    )
                    if adjusted > 0:
                        amount = adjusted
                except Exception:
        ...
        if amount <= 0:
            ...
            exe._mark_opening_completed("non_positive_amount")
            return

        is_profitability_check_passed = exe._check_profitability_enter_condition(
            maker_side, px, amount
        )
        exe.logger().info(
            f"[Order] Placing maker {mode_desc} qty={amount} @ {px:.8f} (profitability check: {'pass' if is_profitability_check_passed else 'fail'})"
        )
        ...
        if exe._closing:
            ...
            if exe._closing_no_wait:
                exe.logger().info(
                    "[Close] No-wait flag set; bypassing profitability wait due to risk trigger."
                )
                try:
                ...
                if not is_profitability_check_passed:
                    ...
                    if start_ts is None:
                        ...
                        exe.logger().info(
                            f"[Close] Not profitable yet; starting up-to-{int(close_wait_sec)}s wait before forcing close."
                        )
                    ...
                    if elapsed_close < close_wait_sec:
                        ...
                        exe.logger().info(
                            f"[Close] Not profitable yet; waiting... remaining {remaining}s before forced close."
                        )
                        exe._next_order_ready_ts = now_ts + 5.0
                        return
                    else:
                        exe.logger().info(
                            f"[Close] Waited {int(elapsed_close)}s; forcing close without profitability condition."
                        )
                else:
                    try:
                        exe._closing_wait_started_ts = None
                ...
                if not exe._profitability_helper.is_profitable_on_last_check:
                    if elapsed >= wait_sec:
                        if not exe.is_any_position_open():
                            exe.logger().info(
                                f"[Profitability] {exe.maker_pair} wait time exceeded ({elapsed:.0f}s >= {wait_sec:.0f}s); no open orders, stopping executor."
                            )
                            exe.early_stop()
                            return
                        exe._mark_opening_completed(
                            "profitability_timeout_enter_monitoring"
                        )
                        exe.logger().info(
                            f"[Profitability] {exe.maker_pair} wait time exceeded ({elapsed:.0f}s >= {wait_sec:.0f}s); entering funding monitoring."
                        )
                    else:
                        exe.logger().info(
                            f"[Profitability] {exe.maker_pair} still not profitable; waiting {wait_sec - elapsed:.0f}s more."
                        )
                    return
                exe._profitability_helper.is_profitable_on_last_check = False
                exe.logger().info(
                    f"[Profitability] {exe.maker_pair} not profitable; starting wait window {wait_sec:.0f}s."
                )
                return
        ...
        order_id = exe.place_order(
            ...
            amount=amount,
            position_action=(
                PositionAction.CLOSE if exe._closing else PositionAction.OPEN
            ),
            price=px,
        )
        ...
        if exe._closing:
            ...
            exe.logger().info(
                f"[Close] Placed maker CLOSE (LIMIT marketable) for open_id={open_id} -> close_id={order_id} px={px} amt={amount}"
            )
            try:
            ...
            exe.logger().info(
                f"[Order] Placed maker {mode_desc} id={order_id} type=LIMIT px={px} amt={amount}"
            )
