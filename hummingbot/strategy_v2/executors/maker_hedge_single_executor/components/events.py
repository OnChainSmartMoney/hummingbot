...
class EventsHelper:
    ...
    def process_order_created(
        ...
        if exe._closing_current and event.order_id == exe._closing_current.get(
            ...
            if updated:
                ...
                exe._closing_current["creation_ts"] = getattr(
                    updated, "creation_timestamp", exe._strategy.current_timestamp
                )
            exe.logger().info(f"[Close] Maker CLOSE acknowledged id={event.order_id}")
            return

        to = exe._maker_by_id.get(event.order_id) or exe._hedge_by_id.get(
            event.order_id
        )
        if to:
            updated = None
    ...
    def process_order_filled(self, event_tag: int, market, event: OrderFilledEvent):
        ...
        if exe._closing_current and event.order_id == exe._closing_current.get(
            ...
            exe._closing_current["executed_base"] = (
        ...
        if event.order_id in exe._hedge_by_id:
            mid_hedge = exe.get_price(
                exe.hedge_connector, exe.hedge_pair, PriceType.MidPrice
            )
    ...
    def process_order_completed(
        ...
        if exe._closing_current and event.order_id == exe._closing_current.get(
            ...
            exe.logger().info(
                f"[Close] Closed open_id={open_id}; removed from maker list. Remaining tracked: {after} (was {before})"
            )
            exe._closing_current = None
            if not exe._close_queue and not any(
                (
                    o.order
                    and o.order.position == PositionAction.OPEN
                    and (o.executed_amount_base or 0) > 0
                )
                for o in exe._maker_orders
            ):
                exe._finalize_hedge_tail()
                exe.logger().info(
                    "[Close] Queue empty and no remaining maker opens; finishing executor."
                )
    ...
    def process_order_canceled(
        self, event_tag: int, market, event: OrderCancelledEvent
    ):
        exe = self.exe
        if (
            exe._closing
            and exe._closing_current
            and event.order_id == exe._closing_current.get("close_order_id")
        ):
            ...
            if remaining > 0:
                ...
                exe.logger().info(
                    f"[Close] Order {event.order_id} canceled; re-queue remaining {remaining} for open_id={exe._closing_current.get('open_id')}"
                )
            else:
                exe.logger().info(
                    f"[Close] Order {event.order_id} canceled; nothing remaining to re-queue."
                )
            exe._closing_current = None
            exe._maker_pending_ids.discard(event.order_id)
            return
        ...
        if event.order_id in exe._maker_by_id:
            ...
            if exec_base <= 0:
                ...
                exe.logger().info(
                    f"[Cancel] Maker order {event.order_id} canceled with partial fill {exec_base}; keeping for exposure accounting."
                )
            ...
            if exec_base <= 0:
                ...
                exe.logger().info(
                    f"[Cancel] Hedge order {event.order_id} canceled with partial fill {exec_base}; keeping for exposure accounting."
                )
    ...
    def process_order_failed(
        self, event_tag: int, market, event: MarketOrderFailureEvent
    ):
        exe = self.exe
        ...
        if exe._closing_current and order_id == exe._closing_current.get(
            "close_order_id"
        ):
            handled = True
            placed = exe._closing_current.get("placed_amount", Decimal("0"))
            ...
            if remaining > 0:
                ...
                exe.logger().warning(
                    f"[Failure] Maker CLOSE order {order_id} failed ({err_msg}); re-queued remaining {remaining}."
                )
            else:
                exe.logger().warning(
                    f"[Failure] Maker CLOSE order {order_id} failed ({err_msg}); nothing remaining to re-queue."
                )
            exe._closing_current = None
        ...
        if maker_tracked is not None:
            handled = True
            exe.logger().warning(
                f"[Failure] Maker order {order_id} failed ({err_msg}); scheduling retry."
            )
        ...
        if hedge_tracked is not None:
            ...
            exe.logger().warning(
                f"[Failure] Hedge order {order_id} failed ({err_msg}); will attempt again."
            )
            if exe._hedge_helper:
                exe._hedge_helper.try_hedge_accumulated()
            ...
            exe.logger().warning(
                f"[Failure] Recovered hedge accumulator {attempted_hedge} for failed order {order_id}; retrying."
            )
            if exe._hedge_helper:
                exe._hedge_helper.try_hedge_accumulated()
        ...
        if not handled:
            exe.logger().warning(
                f"[Failure] Order {order_id} failed ({err_msg}); no tracked state was updated."
            )
