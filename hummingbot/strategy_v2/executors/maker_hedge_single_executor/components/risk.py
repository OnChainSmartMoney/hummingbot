...
class RiskHelper:
    ...
    def ensure_position_mode(self) -> bool:
        ...
        try:
            ...
            desired_mode = getattr(exe, "_desired_hedge_position_mode", PositionMode.HEDGE)
            if desired_mode == PositionMode.HEDGE and mode != PositionMode.HEDGE:
                try:
                    exe._strategy.set_position_mode(exe.hedge_connector, PositionMode.HEDGE)
                except Exception:
                    pass
                exe.logger().warning(f"[Position mode] {exe.hedge_connector} is not in HEDGE mode yet. Waiting before placing orders...")
                return False
            return True
        except Exception:
            exe.logger().warning(f"[Position mode] Unable to verify for {exe.hedge_connector}. Waiting...")
            return False
    ...
    def apply_leverage_once(self):
        ...
        try:
            ...
            try:
                ...
                ok1 = False
                exe.logger().warning(f"[Leverage] Failed to set {lev} on {exe.maker_connector}:{exe.maker_pair}: {e}")
            try:
                exe._strategy.set_leverage(exe.hedge_connector, exe.hedge_pair, lev)
            except Exception as e:
                ok2 = False
                exe.logger().warning(f"[Leverage] Failed to set {lev} on {exe.hedge_connector}:{exe.hedge_pair}: {e}")
