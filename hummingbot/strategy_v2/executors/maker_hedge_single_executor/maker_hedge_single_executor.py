...
class MakerHedgeSingleExecutor(ExecutorBase):
        ...
        try:
            if self._desired_hedge_position_mode == PositionMode.HEDGE:
                try:
                    ...
                    self.logger().info(f"[Position mode] Requested HEDGE on {self.hedge_connector}")
                except Exception as e:
                    self.logger().warning(f"[Position mode] Could not set HEDGE on start: {e}")
            else:
                self.logger().info(f"[Position mode] Using ONEWAY on {self.hedge_connector}")
        except Exception as e:
    ...
    async def validate_sufficient_balance(self):
        ...
        if mid.is_nan() or mid <= 0:
            self.logger().info("[Validation] Entry mid price unavailable; proceeding but orders may fail.")
            return
        ...
        self.logger().info(f"[Validation] connector {self.maker_connector} requires ~{required_margin:.6f} {quote}")
    ...
    def _check_profitability_enter_condition(self, maker_side: TradeType, maker_price: Decimal, amount: Decimal) -> bool:
        return self._profitability_helper.check_enter_condition(maker_side, maker_price, amount)
    ...
    def close_all_positions_by_market(self):
        ...
        self.logger().info(f"[Close] Closing maker via market: {maker_exposure} {self.maker_pair}")
        self.logger().info(f"[Close] Closing hedge via market: {hedge_exposure} {self.hedge_pair}")
