...
class BybitPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        self._real_time_balance_update = (
            False  # Remove this once bybit enables available balance again through ws
        )
    ...
    async def _make_trading_pairs_request(self) -> Any:
        (
            linear_exchange_info_response,
            non_linear_exchange_info_response,
            ...
            self._api_get(
                path_url=self.trading_pairs_request_path,
                params={"category": "inverse", "limit": 1000},
            ),
        )
        for exchange_info_response in [
            linear_exchange_info_response,
            non_linear_exchange_info_response,
        ]:
    ...
    async def _make_trading_rules_request(self) -> Any:
        (
            linear_trading_rules_response,
            non_linear_trading_rules_response,
        ) = await asyncio.gather(
            self._api_get(
    ...
    def _validate_exchange_response(
        self, response: Dict[str, Any], before_text: str = ""
    ):
    ...
    def _is_request_exception_related_to_time_synchronizer(
        ...
        ts_error_target_str = (
            f"{self._format_ret_code_for_print(ret_code=CONSTANTS.RET_CODE_INVALID_TIME)} - "
    ...
    def _is_order_not_found_during_status_update_error(
        ...
        return (
            str(CONSTANTS.RET_CODE_ORDER_NOT_EXISTS) in str(status_update_exception)
            or str(CONSTANTS.RET_CODE_ORDER_NOT_FOUND) in str(status_update_exception)
            or CONSTANTS.RET_MSG_ORDER_NOT_EXISTS in str(status_update_exception)
            or CONSTANTS.RET_MSG_ORDER_NOT_FOUND in str(status_update_exception)
        )
    ...
    def _is_order_not_found_during_cancelation_error(
        ...
        return (
            str(CONSTANTS.RET_CODE_ORDER_NOT_EXISTS) in str(cancelation_exception)
            or str(CONSTANTS.RET_CODE_ORDER_NOT_FOUND) in str(cancelation_exception)
            or CONSTANTS.RET_MSG_ORDER_NOT_EXISTS in str(cancelation_exception)
            or CONSTANTS.RET_MSG_ORDER_NOT_FOUND in str(cancelation_exception)
        )

    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        data = {
            "category": "linear"
            if bybit_utils.is_linear_perpetual(tracked_order.trading_pair)
            else "inverse",
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        self._validate_exchange_response(
            resp, before_text="Error submitting order {order_id}: "
        )
    ...
    async def _update_balances(self):
        ...
        """
        unified_wallet_response = await self._api_get(
    ...
    async def _fetch_available_balance(self, coin: str):
        available_balance_resp = await self._api_get(
            path_url=CONSTANTS.GET_TRANSFERABLE_AMOUNT_PATH_URL,
        ...
        for position in parsed_resps:
            ...
            if self.position_mode == PositionMode.HEDGE:
                if position_idx == CONSTANTS.POSITION_IDX_HEDGE_BUY:
                    ...
                    position_side = (
                        PositionSide.LONG if side_str == "Buy" else PositionSide.SHORT
                    )
            else:
                position_side = (
                    PositionSide.LONG if side_str == "Buy" else PositionSide.SHORT
    ...
    async def _request_order_fills(self, order: InFlightOrder) -> Dict[str, Any]:
        ...
        body_params = {
            "category": "linear"
            if bybit_utils.is_linear_perpetual(order.trading_pair)
            else "inverse",
        ...
        for instrument in instrument_info_dict:
            try:
                ...
                if exchange_symbol in symbol_map:
                    ...
                    collateral_token = (
                        instrument["quoteCoin"] if is_linear else instrument["baseCoin"]
                    )
                    min_notional_size = (
                        Decimal(instrument["lotSizeFilter"]["minNotionalValue"])
                        if is_linear
                        else s_decimal_0
                    )
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in filter(
            bybit_utils.is_exchange_information_valid, exchange_info
        ):
    ...
    def _ensure_registered_limit_id(
        ...
        try:
            if (
                primary_limit_id is not None
                and self._throttler.get_related_limits(primary_limit_id)[0] is not None
            ):
            ...
            if trading_pair is not None:
                ...
                if is_linear:
                    bucket_base = (
                        CONSTANTS.LINEAR_PRIVATE_BUCKET_600_LIMIT_ID
                        if is_get
                        else CONSTANTS.LINEAR_PRIVATE_BUCKET_100_LIMIT_ID
                    )
                else:
                    bucket_base = (
                        CONSTANTS.NON_LINEAR_PRIVATE_BUCKET_600_LIMIT_ID
                        if is_get
                        else CONSTANTS.NON_LINEAR_PRIVATE_BUCKET_100_LIMIT_ID
                    )
                per_pair_bucket_id = web_utils.get_pair_specific_limit_id(
                    bucket_base, trading_pair
                )
                if (
                    self._throttler.get_related_limits(per_pair_bucket_id)[0]
                    is not None
                ):
                    self.logger().debug(
            ...
            global_limit_id = (
                CONSTANTS.GET_LIMIT_ID if is_get else CONSTANTS.POST_LIMIT_ID
            )
            ...
            return (
                CONSTANTS.GET_LIMIT_ID
                if method == RESTMethod.GET
                else CONSTANTS.POST_LIMIT_ID
            )
