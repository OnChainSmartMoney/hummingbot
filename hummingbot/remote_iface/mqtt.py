...
class MQTTCommands:
    def __init__(self, hb_app: "HummingbotApplication", node: Node):
        ...
        topic_prefix = TopicSpecs.PREFIX.format(
            namespace=self._node.namespace, instance_id=self._hb_app.instance_id
        )
        ...
        self._custom_command_uri = f"{topic_prefix}{TopicSpecs.COMMANDS.CUSTOM_COMMAND}"
    ...
    def _init_commands(self):
        ...
        self._custom_command_uri = self._custom_command_uri
        self._node.create_rpc(
            rpc_name=self._custom_command_uri,
            msg_type=CustomCommandMessage,
            on_request=self._on_cmd_custom_command,
        )

    def _on_cmd_custom_command(self, msg: CustomCommandMessage.Request):
        self.logger().info(f"Received custom command: {msg}")
        self.logger().info("hum controller 3")
        response = CustomCommandMessage.Response()
        try:
            self._hb_app.trading_core.strategy.custom_command(
                custom_command=msg.custom_command, params=msg.params
            )
        except Exception as e:
            response.status = MQTT_STATUS_CODE.ERROR
            response.msg = str(e)
            self.logger().error(f"Error processing custom command: {e}", exc_info=True)
        return response
    ...
    def _on_cmd_stop(self, msg: StopCommandMessage.Request):
        ...
        try:
            if msg.async_backend:
                ...
                res = call_sync(
                    self._hb_app.stop_loop(), loop=self._ev_loop, timeout=timeout
                )
                response.msg = res if res is not None else ""
        except asyncio.exceptions.TimeoutError:
            response.msg = f"Hummingbot start command timed out after {timeout} seconds"
    ...
    def _on_cmd_import(self, msg: ImportCommandMessage.Request):
        ...
        try:
            ...
            response.msg = (
                f"Hummingbot import command timed out after {timeout} seconds"
            )
            response.status = MQTT_STATUS_CODE.ERROR
        except Exception as e:
            response.status = MQTT_STATUS_CODE.ERROR
    ...
    def _on_cmd_status(self, msg: StatusCommandMessage.Request):
        ...
        try:
            ...
            if msg.async_backend:
                ...
                res = call_sync(
                    self._hb_app.strategy_status(), loop=self._ev_loop, timeout=timeout
                )
                response.msg = res if res is not None else ""
        except asyncio.exceptions.TimeoutError:
            response.msg = (
                f"Hummingbot status command timed out after {timeout} seconds"
            )
            response.status = MQTT_STATUS_CODE.ERROR
        except Exception as e:
            response.status = MQTT_STATUS_CODE.ERROR
...
class MQTTMarketEventForwarder:
    ...
    def __init__(self, hb_app: "HummingbotApplication", node: Node):
        ...
        self._mqtt_fowarder: SourceInfoEventForwarder = SourceInfoEventForwarder(
    ...
    def _send_mqtt_event(self, event_tag: int, pubsub: PubSub, event):
        if threading.current_thread() != threading.main_thread():  # pragma: no cover
            self._ev_loop.call_soon_threadsafe(
                self._send_mqtt_event, event_tag, pubsub, event
            )
            return
        try:
            event_types = {
        ...
        self.event_fw_pub.publish(
            InternalEventMessage(
                timestamp=int(timestamp), type=event_type, data=event_data
            )
        )
...
class MQTTNotifier(NotifierBase):
    def __init__(self, hb_app: "HummingbotApplication", node: Node) -> None:
        ...
        self.notify_pub = self._node.create_publisher(
            topic=self._topic, msg_type=NotifyMessage
        )
...
class MQTTStatusUpdates:
    ...
    def add_msg_to_queue(self, msg: str, msg_type: str = "hbapp"):
        ...
        self.status_updates_pub.publish(
            StatusUpdateMessage(
                msg=msg, type=msg_type, timestamp=int(time.time() * 1e3)
            )
        )
...
class MQTTGateway(Node):
    ...
    def __init__(self, hb_app: "HummingbotApplication", *args, **kwargs):
        ...
        self._topic_prefix = TopicSpecs.PREFIX.format(
            namespace=self.namespace, instance_id=self._hb_app.instance_id
        )
    ...
    def patch_loggers(self):  # pragma: no cover
        ...
        loggers_filtered = [
            logger for logger in loggers if logger.name in log_conf_names
        ]
        loggers_filtered = [
            logger
            for logger in loggers_filtered
            if log_conf.get("loggers").get(logger.name).get("mqtt", False)
        ]
    ...
    def _remove_notifier(self):
        self._hb_app.notifiers.remove(
            self._notifier
        ) if self._notifier in self._hb_app.notifiers else None
    ...
    def add_external_event_listener(
        self, event_name: str, callback: Callable[[ExternalEventMessage, str], None]
    ):
        if event_name == "*":
    ...
    def remove_external_event_listener(
        self, event_name: str, callback: Callable[[ExternalEventMessage, str], None]
    ):
        if event_name == "*":
    ...
    def _create_mqtt_params_from_conf(self):
        ...
        conn_params = MQTTConnectionParameters(
            host=host, port=int(port), username=username, password=password, ssl=ssl
        )
        return conn_params
    ...
    async def _restart_gateway(self):
        ...
        try:
            ...
            self._hb_app.logger().error(
                f"MQTT Gateway failed to reconnect: {e}. Sleeping 10 seconds before retry."
            )
...
class MQTTLogHandler(logging.Handler):
    def __init__(self, hb_app: "HummingbotApplication", node: Node):
        ...
        self.log_pub = self._node.create_publisher(
            topic=self._topic, msg_type=LogMessage
        )
...
class MQTTExternalEvents:
    ...
    def add_global_listener(
        self, callback: Callable[[ExternalEventMessage, str], None]
    ):
...
class ETopicListener:
    def __init__(
        ...
        self._sub = self._node.create_psubscriber(
...
class ETopicListenerFactory:
    ...
    def create(
        ...
        listener = ETopicListener(
            topic=topic, on_message=callback, use_bot_prefix=use_bot_prefix
        )
        return listener
...
class ETopicQueueFactory:
    ...
    def create(
        ...
        _ = ETopicListener(
            topic=topic, on_message=on_msg, use_bot_prefix=use_bot_prefix
        )
        return queue
