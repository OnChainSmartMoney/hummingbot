...
class V2WithControllers(StrategyV2Base):
    ...
    def __init__(
        self, connectors: Dict[str, ConnectorBase], config: V2WithControllersConfig
    ):
        ...
        self.logger().info(
            f"[Init] Controllers loaded: {list(self.controllers.keys())}"
        )
    ...
    def custom_command(
        ...
        """
        Called from mqtt.py (_on_cmd_custom_command) via self._hb_app.strategy.custom_command(...)

        Here we fan-out this command to controllers that support custom_command().
        """
        self.logger().info("hum strategy 1")
        self.logger().info(
            f"[Manual] Custom command requested on strategy: custom_command={custom_command}, params={params}"
        )
        ...
        for controller_id, controller in self.controllers.items():
            if hasattr(controller, "custom_command"):
                self.logger().info("hum strategy 2")
                try:
                    self.logger().info(
                        f"[Manual] Forwarding custom command to controller={controller_id}, custom_command={custom_command}, params={params}"
                    )
                    executor_ids = controller.custom_command(
                        custom_command=custom_command, params=params
                    )

                    if executor_ids:
                        active_executors = (
                            self.executor_orchestrator.active_executors.get(
                                controller_id, []
                            )
                        )
                        for executor_id in executor_ids:
                            executor = next(
                                (
                                    ...
                                    if e.config.id == executor_id
                                ),
                                None,
                            )
                            if executor:
                                self.logger().info("hum strategy 3")
                                try:
                                    self.logger().info(
                                        f"[Manual] Calling custom command on executor={executor_id}"
                                    )
                                    executor.custom_command(
                                        custom_command=custom_command, params=params
                                    )
                                except Exception as e:
                                    self.logger().error(
                                        f"[Manual] Error calling custom command on executor={executor_id}: {e}",
                                        exc_info=True,
                                    )
                            else:
                                self.logger().warning(
                                    f"[Manual] Executor {executor_id} not found in orchestrator for controller {controller_id}"
                                )

                except Exception as e:
                    self.logger().error(
                        f"[Manual] Error while forwarding custom command to controller={controller_id}: {e}",
                        exc_info=True,
                    )
